...
import FlowReciverNB.RestInterfaceReciver;

import com.northbound.chainreader.ChainReader;
import com.northbound.flowsender.MakeNewFlowDPI;
		...
		while(true){
			...
			if (OUTPUT_PORTS[0] != -1){			// If it is not a ICMPV6 or MDNS packet, that do not have @IPs and @IPd.
				...
				while (j < flowInArray.length){
					...
					} else{							// If the flow have been installed before, we do not install it and we wait for the next flow.
						if (flowIn.getName().equals(flowInArray[j].getName()) &&
